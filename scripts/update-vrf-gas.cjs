...
async function main() {
    console.log("Updating VRF Callback Gas Limit...");
    ...
    console.log(`Current SubId: ${currentSubId.toString()}`);
    console.log(`Current KeyHash: ${currentKeyHash}`);

    const TARGET_GAS = 2500000; // Increase to 2.5M
    if (currentGas.toString() === TARGET_GAS.toString()) {
        console.log(`Gas Limit is already ${TARGET_GAS}. No action needed.`);
        return;
    }
    ...
    // Ensure parameters are correct types for Ethers
